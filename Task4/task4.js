...
function sumSliceArray(arr, first, second) {
    ...
    if (first >= arr.length || second >= arr.length) {
      throw new Error("Both arguments must be less than the length of the array.");
    }
...
function checkAge() {
    try {
      ...
      if (name.trim() === "" || isNaN(age) || status.trim() === "") {
...
Реалізуйте функцію showMonthName(month), в якій параметр month – це порядковий номер місяця в році.
 Функція повертає назву місяця відповідно до введеного номера місяця.
  У випадку некоректного вводу кидається ексепшн у вигляді 
